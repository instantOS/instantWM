...
mod tests {
    ...
    fn globals_with_gaps(inner_gap: i32, outer_gap: i32, smart_gaps: bool) -> Globals {
        let mut globals = Globals::default();
        globals.cfg.layout = LayoutConfig {
            inner_gap,
            outer_gap,
            smart_gaps,
            monocle_gaps: false,
    ...
    fn no_gaps_preserves_slot_except_border() {
        let globals = globals_with_gaps(0, 0, false);
        let monitor = monitor_with_work_rect(Rect::new(0, 0, 100, 80));
        let placement = LayoutPlacement::new(&globals, &monitor, LayoutKind::Tile, 2);
    ...
    fn outer_gap_shrinks_layout_work_rect() {
        let globals = globals_with_gaps(0, 8, false);
    ...
    fn inner_gap_is_split_between_adjacent_slots() {
        let globals = globals_with_gaps(9, 0, false);
        let monitor = monitor_with_work_rect(Rect::new(0, 0, 100, 80));
        let placement = LayoutPlacement::new(&globals, &monitor, LayoutKind::Tile, 2);
    ...
    fn smart_gaps_disable_all_gaps_for_single_tiled_client() {
        let globals = globals_with_gaps(8, 8, true);
        let monitor = monitor_with_work_rect(Rect::new(0, 0, 100, 80));
        let placement = LayoutPlacement::new(&globals, &monitor, LayoutKind::Tile, 1);
    ...
    fn monocle_ignores_gaps_by_default() {
        let globals = globals_with_gaps(8, 8, false);
        let monitor = monitor_with_work_rect(Rect::new(0, 0, 100, 80));
        let placement = LayoutPlacement::new(&globals, &monitor, LayoutKind::Monocle, 3);
