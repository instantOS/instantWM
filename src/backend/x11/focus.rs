...
use x11rb::protocol::xinput::{
    ConnectionExt as XInputConnectionExt, Device, GrabMode22, GrabOwner, GrabType, ModifierMask,
    XIEventMask,
};
...
pub fn grab_buttons(
    ...
    // A core Button1 grab also matches pointer emulation generated by a
    // touchscreen. Put a native XI2 touch grab ahead of it while the client is
    // unfocused. On TouchBegin we focus the window and reject this grab, which
    // replays the original touch sequence to an XI2-aware application instead
    // of replaying it as a mouse drag. Focused clients need no touch grab.
    if !focused && x11_runtime.xi2_touch_grabs {
        let mask = [u32::from(
            XIEventMask::TOUCH_BEGIN | XIEventMask::TOUCH_UPDATE | XIEventMask::TOUCH_END,
        )];
        let modifiers = [u32::from(ModifierMask::ANY)];
        if let Ok(cookie) = conn.xinput_xi_passive_grab_device(
            CURRENT_TIME,
            x11_win,
            x11rb::NONE,
            0,
            Device::ALL_MASTER,
            GrabType::TOUCH_BEGIN,
            GrabMode22::TOUCH,
            GrabMode::ASYNC,
            GrabOwner::NO_OWNER,
            &mask,
            ...
            // Establishing a passive grab has a reply, but focus changes must
            // not synchronously wait for it. Errors are harmless: the core
            // click-to-focus grab remains as the compatibility fallback.
            cookie.discard_reply_and_errors();
        }
    }
