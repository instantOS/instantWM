...
use crate::types::{
    AltCursor, MouseButton, Point, Rect, ResizeDirection, Size, WindowId, get_resize_direction,
};
...
fn resize_target_for_window(
    ...
    Some(HoverResizeTarget {
        win,
        dir: get_resize_direction(c.geo.size(), Point::new(hit_x, hit_y)),
        geo: c.geo,
    })
}
                    ...
                    match btn {
                        ...
                        MouseButton::Left => {
                            if geo.is_at_top_middle_edge(
                                ...
                                let dir =
                                    get_resize_direction(Size::new(w, h), Point::new(win_x, win_y));
