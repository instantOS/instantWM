...
define_named_actions!(
    ...
    Kill => { name: "kill", arg_example: None, doc: "close focused window gracefully", run: |ctx, _args| { if let Some(win) = ctx.core().selected_client() { kill_client(ctx, win); } } },
    ...
    CenterWindow => { name: "center_window", arg_example: None, doc: "center focused window", run: |ctx, _args| { if let Some(win) = ctx.core().selected_client() { center_window(ctx, win); } } },
    ...
    KeyResizeUp => { name: "key_resize_up", arg_example: None, doc: "resize floating window up", run: |ctx, _args| { if let Some(win) = ctx.core().selected_client() { key_resize(ctx, win, VerticalDirection::Up.into()); } } },
    KeyResizeDown => { name: "key_resize_down", arg_example: None, doc: "resize floating window down", run: |ctx, _args| { if let Some(win) = ctx.core().selected_client() { key_resize(ctx, win, VerticalDirection::Down.into()); } } },
    KeyResizeLeft => { name: "key_resize_left", arg_example: None, doc: "resize floating window left", run: |ctx, _args| { if let Some(win) = ctx.core().selected_client() { key_resize(ctx, win, HorizontalDirection::Left.into()); } } },
    KeyResizeRight => { name: "key_resize_right", arg_example: None, doc: "resize floating window right", run: |ctx, _args| { if let Some(win) = ctx.core().selected_client() { key_resize(ctx, win, HorizontalDirection::Right.into()); } } },
    PushUp => { name: "push_up", arg_example: None, doc: "push window up in stack", run: |ctx, _args| { if let Some(win) = ctx.core().selected_client() { reorder_client(ctx, win, VerticalDirection::Up); } } },
    PushDown => { name: "push_down", arg_example: None, doc: "push window down in stack", run: |ctx, _args| { if let Some(win) = ctx.core().selected_client() { reorder_client(ctx, win, VerticalDirection::Down); } } },
    ...
    ToggleSticky => { name: "toggle_sticky", arg_example: None, doc: "toggle sticky (visible on all tags)", run: |ctx, _args| { if let Some(win) = ctx.core().selected_client() { toggle_sticky(ctx, win); } } },
    ...
    Hide => { name: "hide", arg_example: None, doc: "minimize focused window or hide the visible scratchpad", run: |ctx, _args| { if let Some(win) = ctx.core().selected_client() { crate::client::hide_for_user(ctx, win); } } },
