...
pub fn get_keys() -> Vec<Key> {
    let mut keys: Vec<Key> = vec![
        ...
        key!(MODKEY | SHIFT, XK_J => named(NamedAction::KeyMoveDown)),
        key!(MODKEY | SHIFT, XK_K => named(NamedAction::KeyMoveUp)),
        key!(MODKEY | SHIFT, XK_L => named(NamedAction::KeyMoveRight)),
        key!(MODKEY | SHIFT, XK_H => named(NamedAction::KeyMoveLeft)),
        key!(MODKEY, XK_I => named_args(NamedAction::IncMasterCount, &["1"])),
        key!(MODKEY, XK_D => named(NamedAction::DecMasterCount)),
        key!(MODKEY, XK_H => named(NamedAction::MasterFactorShrink)),
        key!(MODKEY, XK_L => named(NamedAction::MasterFactorGrow)),
