...
fn main() {
    let json_str = r#",[{"name":"time","instance":"time","full_text":" 12:34 "}]"#;

    let mut json = json_str.trim();
    if let Some(rest) = json.strip_prefix(',') {
        json = rest.trim_start();
    }

    let single: Result<Vec<RawI3Block>, _> = serde_json::from_str(json);
    println!("{:?}", single);
}
