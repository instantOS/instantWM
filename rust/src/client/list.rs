...
pub fn attach(win: Window) {
    ...
    let old_head = g.monitor(mon_id).and_then(|m| m.clients);
...
pub fn attach_ctx(ctx: &mut WmCtx, win: Window) {
    ...
    let old_head = ctx.g.monitor(mon_id).and_then(|m| m.clients);
...
pub fn detach(win: Window) {
    ...
    for (cur_win, prev_win) in traversal {
        if cur_win == win {
            match prev_win {
                Some(p) => {
                    if let Some(prev_client) = g.clients.get_mut(&p) {
...
pub fn detach_ctx(ctx: &mut WmCtx, win: Window) {
    ...
    for (cur_win, prev_win) in traversal {
        if cur_win == win {
            match prev_win {
                Some(p) => {
                    if let Some(prev_client) = ctx.g.clients.get_mut(&p) {
                        prev_client.next = client_next;
                    }
                }
                None => {
...
pub fn attach_stack_ctx(ctx: &mut WmCtx, win: Window) {
    ...
    let old_stack = ctx.g.monitor(mon_id).and_then(|m| m.stack);
    if let Some(client) = ctx.g.clients.get_mut(&win) {
        client.snext = old_stack;
    }
    if let Some(mon) = ctx.g.monitor_mut(mon_id) {
